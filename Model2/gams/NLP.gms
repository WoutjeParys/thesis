...
qobj..              obj
#					=e=
#						sum((P,T,Z),price_unit(P,T,Z)*demand_unit(P,T,Z)*W(P))*168/card(T)
#						-
...
#					price_unit(P,T,Z) =l= sum(T,price_unit(P,T,Z))/card(T) + LIMIT TODO: werkt niet
					;

priceconstraint2(P,T,Z)..
					price_unit(P,T,Z) =g= P_ref - LIMIT
#					price_unit(P,T,Z) =g= sum(T,price_unit(P,T,Z))/card(T) - LIMIT TODO: werkt niet
					;

priceconstraint3(P,Z)..
#					sum(T,price_unit(P,T,Z)*demand_unit(P,T,Z)) =e= P_ref*demand_tot(P,Z)
...
revenue(P,Z)..
					totalrevenue(P,Z) =e= sum((T),price_unit(P,T,Z)*demand_unit(P,T,Z))*W(P)*168/card(T)
					;
fixedcost(Z)..
