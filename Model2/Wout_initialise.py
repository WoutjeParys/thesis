...
def initialise(length_period):
    ...
    if correction_matrix:
        ...
        sql = 'DROP TABLE IF EXISTS Ownelast;'
        cur.execute(sql)
        sql = 'CREATE TABLE IF NOT EXISTS Ownelast (Hour1 TEXT, Hour2 TEXT, Include_Value FLOAT);'
        cur.execute(sql)
        if test:
            sheetwrite = wbwrite.get_sheet_by_name('Sheet6')
        include_value = list()
        amount_of_days = length_period/24
        for day in range(0,amount_of_days):
            for row in range(1,sh.nrows):
                ...
                for col in range(1,sh.ncols):
                    ...
                    if test:
                        ...
                        # print value
                        writecell.value = value
        cur.executemany('INSERT INTO Ownelast VALUES (?,?,?)', include_value)
        conn.commit()
        print 'Done ownelast'
